...
int evolve(formula &sat, population &p)
{
    ...
    vector<allele> best_individual(p.ind_dim);
    ...
    do
    {
        ...
        best_id = getElite(sat, p, best_individual);
        best_fitness = fitness(sat, best_id, p); 
        cout << best_fitness << " <- current best result" << endl;
        cout << fitness(sat, worst_id, p) << " <- old (worst) result" << endl;
        data_plot.push_back(make_pair((double) (GENERATIONS - counter), (double) best_fitness));
